...
fn test_try_finalize_psbt_outcomes() {
    {
        ...
        let is_final = wallet
            .sign_with_signers(
                &mut psbt,
                &[&get_signers(get_test_wpkh(), &wallet)],
                SignOptions {
                    try_finalize: false,
                    ..Default::default()
                },
...
fn test_try_finalize_psbt_uses_psbt_timelocks() {
    {
        ...
        wallet
            .sign_with_signers(
                &mut psbt,
                &[&get_signers(get_test_single_sig_cltv(), &wallet)],
                SignOptions {
                    try_finalize: false,
                    ..Default::default()
                },
...

    {
        ...
        wallet
            .sign_with_signers(
                &mut psbt,
                &[&get_signers(get_test_single_sig_csv(), &wallet)],
                SignOptions {
                    try_finalize: false,
                    ..Default::default()
                },
...
fn test_create_and_spend_from_truc_tx() -> anyhow::Result<()> {
    ...
    let _ = wallet.sign_with_signers(
        &mut psbt,
        &[
            &get_signers(descriptor, &wallet),
            &get_signers(change_descriptor, &wallet),
        ],
        SignOptions::default(),
    )?;
    let tx_a = psbt.extract_tx()?;
    let txid_a = tx_a.compute_txid();
    let fee_a = wallet.calculate_fee(&tx_a)?;
    ...
    let balance = wallet.balance();
    assert_eq!(
        balance.untrusted_pending + fee_a,
    ...
    let fee_b = wallet.calculate_fee(&tx_b)?;
    ...
    let balance = wallet.balance();
    assert_eq!(
        balance.untrusted_pending +  fee_a + fee_b,
        Amount::from_sat(500_000),
    ...
    let fee_c = wallet.calculate_fee(&tx_c)?;
    ...
    let balance = wallet.balance();
    assert_eq!(
        balance.untrusted_pending + fee_a + fee_b + fee_c,
        Amount::from_sat(500_000),
