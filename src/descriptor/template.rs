...
/// keyring.add_descriptor(KeychainKind::External, P2Pkh(key_external))?;
/// keyring.add_descriptor(KeychainKind::Internal, P2Pkh(key_internal))?;
...
///
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, P2Wpkh_P2Sh(key_external))?;
/// keyring.add_descriptor(KeychainKind::Internal, P2Wpkh_P2Sh(key_internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(
///     wallet
...
///
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, P2Wpkh(key_external))?;
/// keyring.add_descriptor(KeychainKind::Internal, P2Wpkh(key_internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(
///     wallet
...
///
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, P2TR(key_external))?;
/// keyring.add_descriptor(KeychainKind::Internal, P2TR(key_internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(
///     wallet
...
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip44(key.clone(), KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip44(key, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "mmogjc7HJEZkrLqyQYqJmxUqFaC7i4uf89");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "pkh([c55b303f/44'/1'/0']tpubDCuorCpzvYS2LCD75BR46KHE8GdDeg1wsAgNZeNr6DaB5gQK1o14uErKwKLuFmeemkQ6N2m3rNgvctdJLyr7nwu2yia7413Hhg8WWE44cgT/0/*)#5wrnv0xt");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip44Public(key.clone(), fingerprint, KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip44Public(key, fingerprint, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "miNG7dJTzJqNbFS19svRdTCisC65dsubtR");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "pkh([c55b303f/44'/1'/0']tpubDDDzQ31JkZB7VxUr9bjvBivDdqoFLrDPyLWtLapArAi51ftfmCb2DPxwLQzX65iNcXz1DGaVvyvo6JQ6rTU73r2gqdEo8uov9QKRb7nKCSU/0/*)#cfhumdqz");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let key = bitcoin::bip32::Xpriv::from_str("tprv8ZgxMBicQKsPeZRHk4rTG6orPS2CRNFX3njhUXx5vj9qGog5ZMH4uGReDWN5kCkY3jmWEtWause41CDvBRXD1shKknAMKxT99o9qUTRVC6m")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip49(key.clone(), KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip49(key, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "2N4zkWAoGdUv4NXhSsU8DvS5MB36T8nKHEB");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "sh(wpkh([c55b303f/49'/1'/0']tpubDDYr4kdnZgjjShzYNjZUZXUUtpXaofdkMaipyS8ThEh45qFmhT4hKYways7UXmg6V7het1QiFo9kf4kYUXyDvV4rHEyvSpys9pjCB3pukxi/0/*))#s9vxlc8e");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let fingerprint = bitcoin::bip32::Fingerprint::from_str("c55b303f")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip49Public(key.clone(), fingerprint, KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip49Public(key, fingerprint, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "2N3K4xbVAHoiTQSwxkZjWDfKoNC27pLkYnt");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "sh(wpkh([c55b303f/49'/1'/0']tpubDC49r947KGK52X5rBWS4BLs5m9SRY3pYHnvRrm7HcybZ3BfdEsGFyzCMzayi1u58eT82ZeyFZwH7DD6Q83E3fM9CpfMtmnTygnLfP59jL9L/0/*))#3tka9g0q");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let key = bitcoin::bip32::Xpriv::from_str("tprv8ZgxMBicQKsPeZRHk4rTG6orPS2CRNFX3njhUXx5vj9qGog5ZMH4uGReDWN5kCkY3jmWEtWause41CDvBRXD1shKknAMKxT99o9qUTRVC6m")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip84(key.clone(), KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip84(key, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "tb1qhl85z42h7r4su5u37rvvw0gk8j2t3n9y7zsg4n");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "wpkh([c55b303f/84'/1'/0']tpubDDc5mum24DekpNw92t6fHGp8Gr2JjF9J7i4TZBtN6Vp8xpAULG5CFaKsfugWa5imhrQQUZKXe261asP5koDHo5bs3qNTmf3U3o4v9SaB8gg/0/*)#6kfecsmr");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let fingerprint = bitcoin::bip32::Fingerprint::from_str("c55b303f")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip84Public(key.clone(), fingerprint, KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip84Public(key, fingerprint, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "tb1qedg9fdlf8cnnqfd5mks6uz5w4kgpk2pr6y4qc7");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "wpkh([c55b303f/84'/1'/0']tpubDC2Qwo2TFsaNC4ju8nrUJ9mqVT3eSgdmy1yPqhgkjwmke3PRXutNGRYAUo6RCHTcVQaDR3ohNU9we59brGHuEKPvH1ags2nevW5opEE9Z5Q/0/*)#dhu402yv");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let key = bitcoin::bip32::Xpriv::from_str("tprv8ZgxMBicQKsPeZRHk4rTG6orPS2CRNFX3njhUXx5vj9qGog5ZMH4uGReDWN5kCkY3jmWEtWause41CDvBRXD1shKknAMKxT99o9qUTRVC6m")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip86(key.clone(), KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip86(key, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "tb1p5unlj09djx8xsjwe97269kqtxqpwpu2epeskgqjfk4lnf69v4tnqpp35qu");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "tr([c55b303f/86'/1'/0']tpubDCiHofpEs47kx358bPdJmTZHmCDqQ8qw32upCSxHrSEdeeBs2T5Mq6QMB2ukeMqhNBiyhosBvJErteVhfURPGXPv3qLJPw5MVpHUewsbP2m/0/*)#dkgvr5hm");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
...
/// let fingerprint = bitcoin::bip32::Fingerprint::from_str("c55b303f")?;
/// let mut keyring = KeyRing::new(Network::Testnet);
/// keyring.add_descriptor(KeychainKind::External, Bip86Public(key.clone(), fingerprint, KeychainKind::External))?;
/// keyring.add_descriptor(KeychainKind::Internal, Bip86Public(key, fingerprint, KeychainKind::Internal))?;
/// let mut wallet = keyring.into_params()?.create_wallet_no_persist()?;
///
/// assert_eq!(wallet.next_unused_address(KeychainKind::External).expect("keychain must exist").to_string(), "tb1pwjp9f2k5n0xq73ecuu0c5njvgqr3vkh7yaylmpqvsuuaafymh0msvcmh37");
/// assert_eq!(wallet.public_descriptor(KeychainKind::External).expect("keychain must exist").to_string(), "tr([c55b303f/86'/1'/0']tpubDC2Qwo2TFsaNC4ju8nrUJ9mqVT3eSgdmy1yPqhgkjwmke3PRXutNGRYAUo6RCHTcVQaDR3ohNU9we59brGHuEKPvH1ags2nevW5opEE9Z5Q/0/*)#2p65srku");
/// # Ok::<_, Box<dyn core::error::Error>>(())
/// ```
#[derive(Debug, Clone)]
